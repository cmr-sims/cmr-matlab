function seq = gen_tcm_lc_neural(num_trials,param,neural)
% GEN_TCM_LC_2P
% seq = gen_tcm_lc_2p(num_trials,param)
%
% Bare bones version of TCM for exploration. 
% - just one Beta parameter (enc=rec) [B 0-1]
% - first primacy parameter - scale [P1 0-?]
...
% - second primacy parameter - decay [P2 0-?]
%
...
for i=1:num_trials
  ...
  for j = 1:LL
    ...
    % 2 parameter primacy
    P = (param.P1 * exp(-param.P2 * (j - 1))) + 1;

    N = param.N * neural(i,j);
