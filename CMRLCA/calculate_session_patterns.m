...
%  PARAM:
%   subregions
%   pres_indices          - cell array of length [trials]. Each cell
%                           contains a [subregions X items] matrix of
%                           indices of presented items/sources.
%   not_presented_indices
%   do_cdfr
%   cdfr_schedule
%   cdfr_disrupt_regions
%   do_dfr
%   dfr_schedule
%   dfr_disrupt_regions
%   do_shift
%   shift_disrupt_regions
%   shift_trigger_regions
%   do_end_list
%   end_schedule
%   end_disrupt_regions
...
%
%  OUTPUTS:
%               n_patterns:  number of patterns needed.
%
%  first_distraction_index:  index of the first presented distractor.
...
for i = 1:param.subregions
  % get number of unique indices over all trials for this subregion
  unique_labels = length(unique(pres_indices_matrix(i,:)));
  if ~isempty(param.not_presented_indices{i})
    % add recallable items that will not be presented to the network
  ...
  unique_shift = 0;
  if param.do_shift & param.shift_disrupt_regions(i)
  ...
  % to get the number of patterns, add items/sources, distractors, and
  % an initialization index (if applicable)
