function [prec_interv] = prop_interv(rec_intervs,tasks,ll,subjects)
%
% [prec_interv] = PROP_INTERV(rec_intervs,tasks,ll,subjects)
%
%  PROP_INTERV calculates the proportion of target-list items recalled,
...
%
%
%     rec_intervs:  For each trial (row) and output position (column),
%                   a positive integer indicates the serial position of the
%                   intervening item recalled. Technically this function just
%                   looks at the number of values in rec_intervs > 0, so
...
%
%
%     prec_interv:  An nx8 matrix, were n corresponds to the number of 
...
rec_intervs = rec_intervs(task_ind,:);
...
sT_P_sI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,short,short,0,rec_intervs);
lT_P_sI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,long,short,0,rec_intervs);
sT_P_lI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,short,long,0,rec_intervs);
lT_P_lI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,long,long,0,rec_intervs);
sT_R_sI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,short,short,1,rec_intervs);
lT_R_sI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,long,short,1,rec_intervs);
sT_R_lI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,short,long,1,rec_intervs);
lT_R_lI = calc_proportion_recall(ll_t,ll_i,task_bet,subjects,long,long,1,rec_intervs);

% Combine all of these outputs into one vector.
prec_interv = [mean([sT_P_sI lT_P_sI sT_P_lI lT_P_lI],2) ...
	   mean([sT_R_sI lT_R_sI sT_R_lI lT_R_lI],2)];

function prec = calc_proportion_recall(ll_t,ll_i,task_bet,subj,TLL,ILL,have_recall,rec_intervs)
% function prec =
...
recalls_interv_to_use = rec_intervs(rows_to_use,:);
...
for i = 1:nsubj
    ...
    % possible recalls.
    prec(i) = length(find(recalls_interv_to_use(these_rows,:)))/(nlists*ILL);
