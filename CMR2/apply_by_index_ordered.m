function [results, result_index] = apply_by_index_ordered(f, index, dim, matrices, varargin)
% APPLY_BY_INDEX_ORDERED     Applies f on a per-subject, per-session, or 
%                               per-<other index> basis to each matrix M in
%                           matrices. Unlike apply_by_index, this function
%                           maintains the order of subjects by their order
%                           in index, rather than the sorted order given by
%                           taking unique(index).
%
...
%   |2|     |   ...  |  |   ...  |     
   ...
   % initialize outputs
   [temp,first_rows] = unique(index,'first');
   uniq_index = index(sort(first_rows));
   results = []; 
   result_index = [];
